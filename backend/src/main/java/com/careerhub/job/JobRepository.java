...
public interface JobRepository extends JpaRepository<Job, UUID> {
    ...
    @Query("select j from Job j join fetch j.company left join fetch j.skills where j.id = :id")
    ...
    @Query(value = """
           ...
           where j.status = com.careerhub.job.OpeningStatus.PUBLISHED
             ...
             and (:employmentType is null or j.employmentType = :employmentType)
           """)
    Page<Job> searchPublished(@Param("search") String search,
                              @Param("location") String location,
                              @Param("workMode") WorkMode workMode,
                              @Param("employmentType") EmploymentType employmentType,
                              Pageable pageable);

    @Query(value = "select j from Job j join fetch j.company where j.company.id = :companyId",
           countQuery = "select count(j) from Job j where j.company.id = :companyId")
    Page<Job> findByCompany(@Param("companyId") UUID companyId, Pageable pageable);
    ...
    Page<Job> findAllForAdmin(@Param("status") OpeningStatus status, Pageable pageable);
    ...
    @Query("""
           select distinct j from Job j join fetch j.company
           where j.status = com.careerhub.job.OpeningStatus.PUBLISHED
             and exists (select 1 from j.skills s where s.id in :skillIds)
           order by j.createdAt desc
           """)
    List<Job> findRecommended(@Param("skillIds") List<UUID> skillIds, Pageable pageable);

    @Query("""
           select function('to_char', j.createdAt, 'YYYY-MM'), count(j) from Job j
